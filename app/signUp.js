...
export default function SignUp() {
    ...
    const validateForm = () => {
        ...
        if (formData.password.length < 6) {
            Alert.alert('Sign In', "A senha deve conter pelo menos 6 caracteres");
            return false;
        }
    ...
    //Reduz duplicação de código e mante a interface uniforme
    const renderInput = (icon, placeholder, fiels, isSecure = false) => (
        <View style={{height: hp(7)}} className="flex-row gap-4 px-4 bg-neutral-100 items-center rounded-x1">
            ...
            <TextInput
                ...
                onChangeText={(value) => handleChange(field, value)} //atualiza o campo especificado
                style={{fontSize: hp(2)}} //tamanho da fonte responsivo
                className="flex-1 font-semibold text-neutral-700" //estilização com NativeWind
                placeholder={placeholder} //texto de placeholder
                secureTextEntry={isSecure} //determina se o campo
                placeholderTextColor={'gray'} //cor do texto 
